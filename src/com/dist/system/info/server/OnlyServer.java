...
public class OnlyServer extends Observer implements Runnable {
    String host;
    int port;
     ...
     * @param port
     */
    public OnlyServer(String host, int port) {
        super();
    ...
    private void read(AsynchronousSocketChannel socketChannel) {
        ...
        Future<Integer> readFuture = null;
        ...
        try {
            ...
            if(bytesRead == -1) throw new RuntimeException("Error reading buffer.");

            Payload payload = new Payload(readBuffer, bytesRead);
            System.out.println("[Server] Read: " + payload);

            payload.setSocketAddressHeaders(getSocketAddress(socketChannel));
            notifyObservers("server:read", socketChannel, payload);
        } catch (Exception e) {
